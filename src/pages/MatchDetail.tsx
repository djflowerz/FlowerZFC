...
export default function MatchDetail() {
  ...
  // Fetch Live Commentary when switching to commentary sub-filter
  useEffect(() => {
    ...
    <div className="w-full min-h-screen bg-[#0d0d16] text-white flex flex-col font-sans">
      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-4 lg:p-6 space-y-4 max-w-4xl">
          ...
          {/* 1. MATCH HEADER CARD MATCHING LIVESCORE URL DESIGN */}
          <div className="rounded-xl overflow-hidden bg-[#131320] border border-[#1e1e32] shadow-2xl relative">
            ...
            <div className="p-6 flex items-center justify-between gap-6">
              ...
              {/* Score / Kickoff Time Display */}
              <div className="text-center shrink-0">
                {match.isPlayed ? (
                  <>
                    <div className="text-4xl lg:text-5xl font-black text-white mb-0.5 tracking-wider" style={{ fontFamily: 'Big Shoulders Display' }}>
          ...
          {activeTab === 'summary' && match.isPlayed && (
            <div className="space-y-4">
              ...
              {/* TIMELINE EVENTS LIST OR COMMENTARY */}
                ...
                {summarySubFilter === 'events' ? (
                  match.summary.length > 0 ? (
                    match.summary.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-[#1a1a28] border border-[#2a2a40]">
                        <span className="text-xs font-mono font-bold text-gray-400 w-12">{item.min}</span>
                        <div className="flex-1 text-center font-bold text-white">{item.text}</div>
                      </div>
                    ))
                  ) : (
                    <div className="text-center p-8 text-xs text-gray-400 font-mono">
                      {match.status.includes("'") || match.status === 'HT' || match.status === '1H' || match.status === '2H' ? '⚡ Live match in progress — key events will appear here as they happen.' : 'No timeline events available for this match.'}
                    </div>
                  )
                ) : (
                  (commentaryList.length > 0
                    ? commentaryList
                    : match.summary.map(s => ({ minute: s.min, text: s.text, isKeyEvent: true }))
                  ).length > 0 ? (
                    (commentaryList.length > 0
                      ? commentaryList
                      : match.summary.map(s => ({ minute: s.min, text: s.text, isKeyEvent: true }))
                    ).map((c, idx) => (
                      <div key={idx} className={`flex items-start gap-3 p-3 rounded-lg border ${c.isKeyEvent ? 'bg-amber-500/10 border-amber-500/30' : 'bg-[#1a1a28] border-[#2a2a40]'}`}>
                        <span className="text-xs font-mono font-bold text-amber-400 w-12 shrink-0">{c.minute}</span>
                        <p className={`text-xs ${c.isKeyEvent ? 'font-black text-amber-200' : 'text-gray-200'}`}>
                          {c.text.includes('G O O O A A A L') || c.text.includes('provided the assist') || c.text.includes('Yellow card for')
                            ? c.text
                            : (
                                c.text.includes('Goal:')
                                  ? `G O O O A A A L - ${c.text.replace(/.*Goal:\s*/, '').replace(/\s*\[.*\]/, '').trim()} is on target!`
                                  : c.text.includes('Assist:')
                                  ? `${c.text.replace(/.*Assist:\s*/, '').replace(/\s*\[.*\]/, '').trim()} provided the assist for the goal.`
                                  : c.text.includes('Yellow Card:')
                                  ? `Yellow card for ${c.text.replace(/.*Yellow Card:\s*/, '').trim()}.`
                                  : c.text.includes('Red Card:')
                                  ? `Red card for ${c.text.replace(/.*Red Card:\s*/, '').trim()}.`
                                  : c.text
                              )
                          }
                        </p>
                    ...
                    <div className="text-center p-8 text-xs text-gray-400 font-mono">
                      🎙️ Match commentary will update as events happen.
                    </div>
                  )
                )}
              </div>
